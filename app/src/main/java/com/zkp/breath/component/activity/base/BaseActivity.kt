...
abstract class BaseActivity : AppCompatActivity() {
    ...
    private fun circularReveal(intent: Intent?) {
        ...
        if (intent?.sourceBounds == null && intent?.hasExtra(CLICK_X)?.not() ?: true) return
        ...
        onGlobalLayout = object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {//此时既是开始揭露动画的最佳时机
                ...
                mAnimReveal?.addListener(object : AnimatorListenerAdapter() {
    ...
    private fun circularRevealReverse(intent: Intent?) {
        // 当我们获取不到上个界面的点击区域时就不展示揭露动画，因为此时没有合适的锚点
        if (intent?.sourceBounds == null && intent?.hasExtra(CLICK_X)?.not() ?: true) {
            super.onBackPressed()
            return
        }
        val rect = intent?.sourceBounds
        ...
        mAnimRevealR?.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(animation: Animator?) {
