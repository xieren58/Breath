...
/**
 * 1.非内联函数的lambda表达式是不允许直接return，只能使用"@+标签"退出lambda表达式。
 * 2.内联函数的无返回值lambda表达式可以直接return，也可以使用"@+标签"退出lambda表达式。
 * 3.内联函数的有返回值lambda表达式可以直接return，但不能使用"@+标签"退出lambda表达式。
 */
fun return1() {
    // 因为该函数的lambda存在返回值
