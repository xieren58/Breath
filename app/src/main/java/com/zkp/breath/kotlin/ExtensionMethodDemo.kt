 ...
 * 2.成员扩展函数：既是外部类的成员函数，又是前缀类的扩展函数，只是由于它同时还是成员函数，所以只能在它所属的类里面被
 *   调用，到了外面就不能用了。
 ...
 * 2. 扩展属性不能有初始化器，没有后台字段field，所以var的set访问器没有实际作用意义，所以一般用val修饰符
 * 3. 其实扩展变量和扩展方法作用一样，因为变量的get访问器其实就是一个方法。有些东西写成扩展属性比扩展函数
 *    更加直观。
 */
